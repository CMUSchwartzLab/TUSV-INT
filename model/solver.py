#   author: Jesse Eaton, Xuecong Fu, and Nishat Bristy
...
def get_U(F_phasing, C, n, l, only_leaf, ancestral):
    ...
    mean_F = gp.quicksum(F_phasing[p, s] for p in range(0, m) for s in range(0, L)) / (m * L)
    mean_UC = gp.quicksum(U[p, k] * C[k, s] for p in range(0, m) for k in range(0, N) for s in range(0, L)) / (m * L)

    sums = []
    for p in range(0, m):
        for s in range(0, L):
            f_hat = gp.quicksum([U[p, k] * C[k, s] for k in range(0, N)])  # UC[p, s]
            sums.append((F_phasing[p, s] - mean_F) * (f_hat - mean_UC))
    reg_term = gp.quicksum(U[p, k] * U[p, k] for p in range(0, m) for k in range(0, N))
...
def _set_matching_constraints(mod, M, ancestral):
    ...
    if not ancestral:
        for i in range(0, N):
            mod.addConstr(sum(M[i,j] for j in range(0,N_r)) == 1) # each DNA clone must have a matching RNA clone
        for i in range(0, N_r):
            mod.addConstr(sum(M[j,i] for j in range(0,N)) <= 1) # all RNA clones might not have a matched DNA clone.
    else: 
        for i in range(0, N):
            mod.addConstr(sum(M[i,j] for j in range(0,N_r)) == 1) # each DNA clone must have a matching RNA clone (best matching RNA clone)
        for i in range(0, N_r):
            mod.addConstr(sum(M[j,i] for j in range(0,N)) >= 0) # some RNA clones might not have any matched DNA clone, but some might have more than one.
...
def _get_objective(mod, F_phasing, U,M, C,C_RNA, R, S, lamb1, lamb2, l_g):  # returns expression for objective
    ...
    for i in range(0, N):
        for j in range(0, r):
            c_hat = gp.quicksum([M[i, k] * C_RNA[k, j] for k in range(0, N_r)])
