...
def unmix(in_dir, out_dir, scrna_file, n, c_max, lamb1, lamb2, num_restarts, num_cd_iters, num_processors, time_limit, metadata_fname, \
    ...
    if not multi_num_clones:
        ...
        M_pre = copy.deepcopy(Ms[best_i]) ### nb
        if collapse:
            U_best, M_best, C_best, E_best, A_best, R_best, W_best, W_SV_best, W_SNV_best = collapse_nodes(Us[best_i], Ms[best_i], Cs[best_i], Es[best_i], As[best_i], Rs[best_i], Ws[best_i], W_SVs[best_i], W_SNVs[best_i], threshold,only_leaf)
        else:
        ...
        write_to_files(out_dir, l_g, U_best, M_best, C_best, E_best, R_best, W_best, W_SV_best, W_SNV_best, W_unsampled, W_con, obj_vals[best_i], F_phasing_full, F_unsampled_phasing_full, org_indxs, writer, E_pre, R_pre, W_pre, M_pre, B, A_best)
    else:
        training_obj = np.zeros(n-1)
        for n_ in range(2, n+1):
            U, M, C, E, A_, R, W, W_SV, W_SNV, obj_val, err_msg = sv.get_UCE(F_phasing, C_RNA, Q, G, A, H, n_, c_max, lamb1,
            ...
            #                                 unsampled_snv_list_sort) # nb: commented
...
def concatenate_W(W_SV_TUSV, W_SV_MATCHING, W_SNV_TUSV, W_SNV_MATCHING, sampled_sv_list_sort, unsampled_sv_list_sort, sampled_snv_list_sort, unsampled_snv_list_sort, l_ab_s, g_ab_s,l_ab_un, g_ab_un):
    ...
    if l_unsampled != 0:
        ...
        W_con[:, sampled_sv_list_sort_idx] = W_SV_TUSV # nb
        W_con[:, unsampled_sv_list_sort_idx] = W_SV_MATCHING # nb
    else:
        #W_con[:, :l]= W_SV_TUSV # NISHAT COMMENTED
        W_con[:, sampled_sv_list_sort] = W_SV_TUSV # NISHAT ADDED
    if g_sampled != 0:
        #sampled_snv_list_sort_idx = [i for i in range(g_sampled)] # nb
        #unsampled_snv_list_sort_idx = [i for i in range(g_sampled, g_sampled + g_unsampled)] # nb
        #W_snv_con[:, sampled_snv_list_sort_idx] = W_SNV_TUSV
...
# collapse nodes
def collapse_nodes(U, M, C, E, A, R, W, W_SV, W_SNV, threshold=0.0, only_leaf=False):
    # generate the tree
    tree = ModifyTree(E)
    if not only_leaf:
    ...
    M_new = np.delete(M, remove_idx, axis=0)
    M_new = np.delete(M, remove_idx, axis=1)
    ...
    return U_new, M_new, C_new, E_new, A_new, R_new, W_new, W_SV_new, W_SNV_new
...
def write_to_files(d, l_g, U, M, C, E, R, W, W_SV, W_SNV, W_SNV_UNSAMPLED, W_con, obj_val, F_phasing_full, F_unsampled_phasing_full, org_indices, writer, E_pre, R_pre, W_pre, M_pre, B, A):
...
def get_args(argv):
    ...
    parser.add_argument('-f', '--scRNA_file', required = True, type = lambda x: fm.is_valid_file(parser,x), help = '.tsv file containing clone CNVs in each row and chr_start_end_major/minor as column names')
